...
* User must be able to create a simple guest registration (Name and Email address), in order to use the star icon/favorites (data is only stored in local storage and session based)
* The star icon (add/remove) is used to save the current episode to favorites list/page
* Episodes are displayed in a listing. (As on the wireframe)
* On click on an episode, a new screen/page must show all available information regarding this episode (Full plot text/Actors/Writers ...) The UI is up to you.
